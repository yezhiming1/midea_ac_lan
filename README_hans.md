...
> 本兼容分支跟随上游项目，并为华凌 `220F4047`、子类型 `8` 保留已通过实机核对的开关机、风吹人/风避人、智能光感、屏显和声音控制，同时加入基于公开 Lua 源码的摆风、固定出风方向、干燥、智控温和省电候选；新增候选只有在目标设备逐项回读通过后才视为已验收。未选择或未证实支持的可选实体会由集成可恢复地禁用，不会删除实体记录；以后在“选项”中重新勾选即可恢复。室外温度及 App 智能服务、智定义、智能场景、定时、用电图表和完整故障流程不在本分支适配范围内。
